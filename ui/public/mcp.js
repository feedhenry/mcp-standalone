...
var resolveMCPRoute = {
  'MCPRoute': ['$route', 'ProjectsService', 'DataService', function($route, ProjectsService, DataService) {
    return ProjectsService
      .get($route.current.params.project)
      .then(_.spread(function(project, context) {
        return DataService
          .get("routes", 'mcp-standalone', context, {errorNotification: false})
          .then(function(route) {
                window.MCP_URL = (route.spec.tls ? 'https://' : 'http://') + route.spec.host;
          });
    }));
  }]
};
...
angular
  ...
  .service('mcpApi', ['$http','AuthService',function ($http,AuthService) {
    ...
    return {
      "mobileApps" : function(){
        return $http.get(getMobileAppsURL(),requestConfig)
        .then((res)=>{
          return res.data;
        })
      },
      "mobileApp": function(id){
        return $http.get(getMobileAppsURL() + "/"+id,requestConfig)
        .then((res)=>{
          return res.data;
        })
      },
      "createMobileApp":function(mobileApp){
        return $http.post(getMobileAppsURL(),mobileApp,requestConfig)
        .then((res)=>{
          return res.data;
        })
      },
      "mobileServices": function(){
        return $http.get(getMobileServicesURL(),requestConfig)
        .then((res)=>{
          return res.data;
        })
      }
  ...
  .controller('MobileAppsController', [
    ...
    'ProjectsService',
    'mcpApi',
    function($scope, $routeParams, $location, DataService, ProjectsService, mcpApi) {
    ...
    $scope.services = [];
      ...
      $scope.openApp = function (id) {
        $location.path("project/" + $routeParams.project + "/browse/mobileapps/" + id);
      };
    }
  ])
  .controller('MobileAppController', [
  ...
  .controller('CreateMobileappController', ['$scope', '$routeParams', '$location', 'mcpApi', function ($scope, $routeParams, $location, mcpApi) {
    $scope.alerts = {};
    $scope.projectName = $routeParams.project;
    ...
    $scope.createApp = function(){
      ...
      .then((app)=>{
        $location.path("project/" + $routeParams.project + "/browse/mobileapps");
      })
      .catch(err =>{
